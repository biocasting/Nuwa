...
namespace Nuwa
{
    class cXmlIO
    {
        ...
        //根据节点 返回子节点的Name值，用于判断
        public string GetChildData(string fatherNode,string node)
        {
            XmlNodeList nodeList = objXmlDoc.SelectSingleNode(fatherNode).ChildNodes;
            foreach (XmlNode xn in nodeList)
            {
                XmlNode xe = xn.FirstChild;
                if (xe.Name== node)
                {
        ...
        //查看节点是否存在
        public string IsExist(string nodename)
        {
            XmlNode node = objXmlDoc.SelectSingleNode(nodename);
            if (node != null)
            {
